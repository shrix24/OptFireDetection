...
class VideoServer_functions():
    ...
    def generate_sdp_file(self):
        try:
            with open(r"C:\development\XPrizev3\test_2.sdp","w+")as fp:
                fp.truncate(0)
            fp.close()
            f = open(r"C:\development\XPrizev3\test_2.sdp", "a")
...
def video_server_connection_thread():
    ...
    try:
        ...
        # ffmpeg_cmd = shlex.split(f'ffmpeg -nostats -nostdin -probesize 1M -flags low_delay -fflags nobuffer -protocol_whitelist file,udp,rtp -i test_2.sdp -pix_fmt bgr24 -an -c:v libx264rgb -b:v 1024k -preset slower -r 15 -f rawvideo pipe:')
        ffmpeg_cmd = shlex.split(f'ffmpeg -nostats -protocol_whitelist "file,rtp,udp" -i test_2.sdp -pix_fmt bgr24 -b:v 1024k -f rawvideo pipe:')
        # ffmpeg_cmd = shlex.split(f'ffmpeg -nostats -nostdin -probesize 1M -flags low_delay -fflags nobuffer -protocol_whitelist file,udp,rtp -i test_2.sdp -pix_fmt bgr24 -an -c:v libx264rgb -b:v 1024k -preset slower -r 15 -f rawvideo pipe:')
...
def video_server_main_thread():
    ...
    # print("Detector Model Chosen: ", global_vars.detector_model)
        ...
        try:
            ...
            if len(raw_frame) != (width*height*3):
                ...
                if global_vars.detection_process_flag == 1:
                    ...
                    image = frame.reshape((global_vars.height, global_vars.width,3), order="C")
    ...
    global_vars.video_server.close_video_stream()
    print("Video Server connection ended peacfully")
